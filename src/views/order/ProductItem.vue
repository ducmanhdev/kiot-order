<template>
  <div class="cursor-pointer">
    ...
    >
      <img class="w-full h-full object-contain rounded-[24px]" :src="data.image" alt="" v-if="data.image">
    ...
    <div class="flex gap-4">
      <p class="grow text-24px font-medium w-break-words">{{ data.name }}</p>
      <div class="shrink-0 text-right">
        <p class="text-24px text-primary font-semibold">{{ FormatHelper.formatShownPrice(data.price) }}</p>
      </div>
    </div>
  </div>
</template>
