...
export async function printReceipt(cartList: any[], checkoutResponse: any) {
    try {
        const orderStore = useOrderStore();
        const encoder = new EscPosEncoder();

        const modifierValueRaw = (modifierValue: any) => {
            return modifierValue.map((item: any) => {
                    return (
                        encoder
                            .initialize()
                            .line(`- ${item.name}`)
        ...
        const modifierRaw = (modifier: any) => {
            return modifier.map((item: any) => {
                    return (
                        encoder
                            .initialize()
